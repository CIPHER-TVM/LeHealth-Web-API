...
namespace LeHealth.Core.DataManager
{
    public class TodaysPatientManager : ITodaysPatientManager
    {
        ...
        public List<SearchAppointmentModel> GetAllAppointments(AppointmentModel appointment)
        {
            ...
            if ((dtAppointmentsList != null) && (dtAppointmentsList.Rows.Count > 0))
            {
                for (Int32 i = 0; i < dtAppointmentsList.Rows.Count; i++)
                {
                    ...
                    {
                        AppId = Convert.ToInt32(dtAppointmentsList.Rows[i]["AppId"]),
                        AppDate = dtAppointmentsList.Rows[i]["AppDate"].ToString(),
                        AppType = Convert.ToInt32(dtAppointmentsList.Rows[0]["AppTypeId"]),
                        AppNo = dtAppointmentsList.Rows[i]["AppNo"].ToString(),
                        PatientId = Convert.ToInt32(dtAppointmentsList.Rows[0]["PatientId"]),
                        PatientName = dtAppointmentsList.Rows[i]["PatientName"].ToString(),
                        PatientRegNo = dtAppointmentsList.Rows[i]["PatientRegNo"].ToString(),
                        PIN = dtAppointmentsList.Rows[i]["PIN"].ToString(),
                        Mobile = dtAppointmentsList.Rows[i]["Mobile"].ToString(),
                        ConsultantName = dtAppointmentsList.Rows[i]["ConsultantName"].ToString(),
                        AppStatus = dtAppointmentsList.Rows[i]["AppStatus"].ToString(),
                        ResPhone = dtAppointmentsList.Rows[i]["ResPhone"].ToString(),
                        Address1 = dtAppointmentsList.Rows[i]["Address1"].ToString()
                    };
                ...
                using (SqlCommand cmdslot = new SqlCommand("stLH_GetSliceDataByAppId", con))
                {
                    ...
                    if ((dt != null) && (dt.Rows.Count > 0))
                    {
                        for (Int32 i = 0; i < dt.Rows.Count; i++)
                        {
                            Slice rsm = new Slice
                            {
                                SliceNo = Convert.ToInt32(dt.Rows[i]["SliceNo"]),
                                SliceTime = dt.Rows[i]["SliceTime"].ToString()
                            };
                            slicelist.Add(rsm);
                        }
                    }
        ...
        public List<PatientListModel> SearchPatient(PatientSearchModel patientDetails)
        {
            ...
            cmd.Parameters.AddWithValue("@RegNo", patientDetails.RegNo.ToUpper().Trim());
        ...
        public List<PatientModel> GetPatient(Int32 patientId)
        {
            ...
            if ((dtPatientList != null) && (dtPatientList.Rows.Count > 0))
            {
                for (Int32 i = 0; i < dtPatientList.Rows.Count; i++)
                {
                    ...
                    {
                        PatientId = Convert.ToInt32(dtPatientList.Rows[i]["PatientId"]),
                        RegNo = dtPatientList.Rows[i]["RegNo"].ToString(),
                        FirstName = dtPatientList.Rows[i]["FirstName"].ToString(),
                        MiddleName = dtPatientList.Rows[i]["MiddleName"].ToString(),
                        LastName = dtPatientList.Rows[i]["LastName"].ToString(),
                        Hook = dtPatientList.Rows[i]["Hook"].ToString(),
                        NationalityId = Convert.ToInt32(dtPatientList.Rows[i]["NationalityId"])
                    };
            ...
            if (obj.ConsultationId != 0)
            {
                ...
                if ((dt != null) && (dt.Rows.Count > 0))
                {
                    for (Int32 i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            SymptomId = Convert.ToInt32(dt.Rows[i]["SymptomId"]),
                            NewsymDesc = dt.Rows[i]["SymptomDesc"].ToString()
                        };
